...
export async function GET() {
  ...
  try {
    ...
    // return NextResponse.json({ result: itemList, value: null });
    return NextResponse.json({
      // parseInt(lastElem.value) > 0 ? 1 : 0;
      result: parseInt(itemList[itemList.length - 1].value) > 0 ? 1 : 0,
      value: itemList[itemList.length - 1].value,
    });
  } catch (error) {
    try {
      const now = new Date();
